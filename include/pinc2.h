...
/// @param framebuffer_index the index of the framebuffer, starting at 0. The allowed range for this is from 0 to the output of pinc_framebuffer_format_get_num-1.
/// @return 0 if there is no depth buffer, otherwise the number of bits in the depth buffer. Usually 24 on modern systems.
...
// window properties:
// r -> can be read at any time. It has a default for before completion [default is in square brackets]
// rc -> can only be read after completion or after being set by the user (the default is determined at completion)
